...
public abstract class DevTTY extends DevBase {
	...
	protected static final byte CL = 0x0C;
	protected static final byte CR = 0x0D;
	
	protected static final byte DLE = 0x10;//跳出資料通訊
	protected static final byte PER = 0x25;//可能是表示暫停通訊的意思，忙碌中～～
	...
	protected int protocol_3964R_express(
		...
		try {
			...
			byte cc = dev.readBytes(1)[0];
			if(cc!=DLE) {
				Misc.loge("[3964R_express]: no DLE begin(%02X)",cc);
				return -10;
			}
			do {				
				byte[] dd = dev.readBytes(2);
				if(dd[0]==PER && dd[1]==STX) {
					//wait device
					dev.writeByte(STX);
				}else if(dd[0]==PER && dd[1]==STX) {
					//all ya~~~ goto next step~~~
					dev.writeByte(DLE);
